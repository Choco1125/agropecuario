...
<!-- Modal para editar -->
<div class="modal fade" id="edit_maquinaria" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title"> Agregar al inventario de Máquinaria y equipo </h5>
        ...
        <div class="modal-body">
          <form id="edit" method="POST">
            ...
            <div class="form-group">
            <label for="articulo"> Articulo </label>
            <input name="articulo" type="text" class="form-control" id="edit_articulo" placeholder="Nombre del Articulo">
            </div>

  			<div class="form-group">
  			<label for="valor"> Valor </label>
  			<input name="valor" type="text" class="form-control" id="edit_valor" placeholder="Valor del articulo ">
  			</div>

  			<div class="form-group">
  			<label for="fecha"> Fecha de ingreso </label>
  			<input name="fecha" type="date" class="form-control" id="edit_fecha" placeholder="Ingrese la fecha de ingreso">
  			</div>

  			<div class="form-group">
  			<label for="n_factura"> Número de factura </label>
  			<input name="n_factura" type="text" class="form-control" id="edit_n_factura" placeholder="Ingrese el número de la factura">
  			</div>

  			<div class="form-group">
  			<label for="cantidad"> Cantidad </label>
  			<input name="cantidad" type="number" class="form-control" id="edit_cantidad" placeholder="Ingrese la cantidad">
  			</div>

  			<div class="form-group">
  			<label for="observacion"> Observaciones </label>
  			<textarea name="observacion" id="edit_observacion" cols="30" rows="10" class="form-control"></textarea>
  			</div>
	      ...
	      <div class="modal-footer">
	        <button type="button" class="btn btn-danger" onclick="limpiar();" data-dismiss="modal"> Cerrar </button>
	        <button type="button" onclick="save();" class="btn btn-success"> Agregar </button>
	      </div>
    </div>
  </div>
</div>
<!-- Cierre de modal para editar -->
...
<div class="container contenedor">
	...
	<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#add_maquinaria">
	Agregar
	</button>
	...
	?>
	  <tr>
	    ...
	    <td> 	 
	      <a href="#" data-target="#edit_maquinaria" onclick="read(<?php echo $row['id']; ?>)"
...
<script>
	...
	function verificar_e(){
		var articulo = $('#edit_articulo');
		var valor = $('#edit_valor');
		var fecha = $('#edit_fecha');
		var n_factura = $('#edit_n_factura');
		var cantidad = $('#edit_cantidad');
		var observacion = $('#edit_observacion');
		...
		if (verificar_e()) {
			...
			.done(function(data) {
				if (data == "1") {
					...
					swal ("Error", "No se logro editar el registro","error");
				}
			});			
		}
	          ...
	          if (res) {
				...
				.done(function(data) {
					if (data == "1") {
				        ...
				        swal ("Error", "No se logro eliminar el registro","error");
				      }
					});
	           }
        });		
