...
<?php 
  ...
  $query = "SELECT * FROM rubros_otros";
...
<div class="container contenedor">
  ...
  <div class="row">
    ...
    <div class="col-10">
      <h3 class="text-center"> Presupuesto / Otros Gastos </h3>
    </div>
  </div>
  <br>
    ...
    <th scope="col"> Observaciones </th>
		...
		<tbody>
			<?php 
				$query = "SELECT g.fecha,g.n_factura,r.nombre,g.valor,g.observacion FROM gastos_otros AS g INNER JOIN rubros_otros AS r WHERE g.rubro_id = r.id AND g.tipo = 0 AND g.finca_id=".$_SESSION['finca'];
				$sql = mysqli_query($conection, $query);
				if ($sql) {
					while ($row = mysqli_fetch_array($sql)) {
						?>
...
function add(){
    ...
    $.ajax({
      ...
      type: 'POST',
      data: $('#add').serialize()+'&op=add_otros',
    })
    .done(function(data) {
      if (data == '1') {
