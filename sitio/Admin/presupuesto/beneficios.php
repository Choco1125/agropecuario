...
<div class="container contenedor">
  ...
  <div class="row">
    ...
    <div class="col-10">
      <h3 class="text-center"> Presupuesto / Beneficios Post-Cosecha </h3>
    </div>
  </div>
  <br>
    ...
    <th scope="col"> Observaciones </th>
		...
		<tbody>
			<?php 
				$query = "SELECT g.fecha,g.n_factura,r.nombre,c.nombre AS 'cultivo',g.valor,g.observacion FROM gastos_post AS g INNER JOIN rubros_post AS r INNER JOIN cultivo as c WHERE g.rubro_id = r.id AND g.tipo = 0 AND c.id = g.cultivo_id  AND g.finca_id=".$_SESSION['finca'];
				$sql = mysqli_query($conection, $query);
				if ($sql) {
					while ($row = mysqli_fetch_array($sql)) {
						?>
